...
class KennConfig:
    def __init__(self,
                 ...
                 cnn_kernel_size=129//2+1,  # must be odd!!
...
class KennAttentionHead(nn.Module):
    ...
    def __init__(
        ...
        self.values = nn.Linear(self.d_model, self.head_dim) #, bias=bias) # shouldn't the second dimension be d_model??
...
class KennConvolutionalEmbedding(nn.Module):
    def __init__(self, config: KennConfig):
        ...
        self.tokenizer = nn.Sequential(self.conv1, self.linear, config.activation_f())
...
class KennFeatureExtractor(nn.Module):
    ...
    def __init__(self, config: KennConfig):
        ...
        self.norm = nn.BatchNorm1d(config.n_channels)
    def forward(self, x):
        #x = self.norm(x)
...
class KennClassifier(nn.Module):
    def __init__(self, config: KennConfig):
        ...
        # classification between 4 classes
        self.classification = nn.LazyLinear(4)
    ...
    def forward(self, x):
        ...
        predictions = self.classification(x).softmax(dim=1)
