...
if __name__ == '__main__':
    ...
    stats_dir = f"{SAVING_PATH}/stats2"
    ...
    with torch.device(device):
        ...
        # we initialize a dictionary (a new one yay) with lists, to save every pearsons
        for det in ['E1', 'E2', 'E3']:
            for i_bbh in range(num_overlapping_signals['BBH']):
                pearson_coeff[f'{det}_{i_bbh}'] = []
        ...
        for i in tqdm(range(NUM_POSTERIORS)):
            ...
            # create sum of the templates
            sum_templates = {}
            for det in ['E1', 'E2', 'E3']:
                sum_templates[det] = torch.sum(templates_oredered[det], axis=0)
            ...
            for det in ['E1', 'E2', 'E3']:
                ...
                dummy_pearson = 0
                for i_bbh in range(num_overlapping_signals['BBH']):
                    dummy_subtraction = sum_templates[det] - templates_oredered[det][i_bbh]
                    #pearson
                    pears = pearson_correlation(templates_oredered[det][i_bbh], dummy_subtraction)
                    pearson_coeff[f'{det}_{i_bbh}'].append(pears)
...
'''
    ...
    df.to_csv(f"{SAVING_PATH}/stats2/stats_withpearson.csv", index=False)


    # save errors on another csv file
    error_df.to_csv(f"{SAVING_PATH}/stats2/errors_withpearson.csv", index=False)
    ...
    mode_df.to_csv(f"{SAVING_PATH}/stats2/mode.csv", index=False)
    median_df.to_csv(f"{SAVING_PATH}/stats2/median.csv", index=False)
    truth_df.to_csv(f"{SAVING_PATH}/stats2/truth.csv", index=False)

    # pearson coefficients

    pearson_df = pd.DataFrame.from_dict(pearson_coeff)
    pearson_df.to_csv(f"{SAVING_PATH}/stats2/pearson_coefficients_1vs2.csv", index=False)
